package Ex6;

public class Main {

    public static void main(String[] args) {

        // Tạo user
        User u1 = new User(1, "dev_a", "123456");
        User u2 = new User(2, "dev_b", "abcdef");
        User u3 = new User(3, "dev_c", "111111");

        // Thêm vào hệ thống
        UserManager.addUser(u1);
        UserManager.addUser(u2);
        UserManager.addUser(u3);

        System.out.println(">> Đã thêm 3 user vào hệ thống.");
        System.out.println("Danh sách hiện tại:");

        // In danh sách
        UserManager.showAllUsers();

        System.out.println("\n>> Kiểm tra đăng nhập:");

        // Test login đúng
        boolean result1 = UserManager.checkLogin("dev_a", "123456");
        System.out.println("- Login (\"dev_a\", \"123456\"): "
                + (result1 ? "Thành công!" : "Thất bại!"));

        // Test login sai
        boolean result2 = UserManager.checkLogin("dev_b", "sai_pass");
        System.out.println("- Login (\"dev_b\", \"sai_pass\"): "
                + (result2 ? "Thành công!" : "Thất bại!"));

        // ===== Thử final (Optional) =====
        // u1.id = 999; // ❌ Sẽ lỗi biên dịch
    }
}
